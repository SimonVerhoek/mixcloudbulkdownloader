...
from app.utils.ffmpeg import get_ffmpeg_path, get_ffprobe_path, verify_ffmpeg_availability
...
class TestGetFFmpegPath:
    ...
    @patch('platform.system')
    def test_get_ffmpeg_path_macos(self, mock_system):
        ...
        # Verify the path structure
        assert str(result).endswith("resources/ffmpeg/macos/ffmpeg")
        assert "app" in str(result)
        assert result.name == "ffmpeg"

    @patch('platform.system')
    def test_get_ffmpeg_path_case_insensitive(self, mock_system):
    ...
    @patch('platform.system')
    def test_get_ffmpeg_path_base_directory_structure(self, mock_system):
...
class TestGetFFprobePath:
    ...
    @patch('platform.system')
    def test_get_ffprobe_path_macos(self, mock_system):
        ...
        # Verify the path structure
        assert str(result).endswith("resources/ffmpeg/macos/ffprobe")
        assert "app" in str(result)
        assert result.name == "ffprobe"

    @patch('platform.system')
    def test_get_ffprobe_path_case_insensitive(self, mock_system):
        """Test that platform detection is case-insensitive."""
        test_cases = [
            ("WINDOWS", "ffprobe.exe"),
            ("windows", "ffprobe.exe"),
...
class TestFFmpegFFprobePathConsistency:
    ...
    @patch('platform.system')
    def test_ffmpeg_ffprobe_same_directory_macos(self, mock_system):
        ...
        assert ffmpeg_path.parent == ffprobe_path.parent
        assert ffmpeg_path.parent.name == "macos"

    @patch('platform.system')
    def test_ffmpeg_ffprobe_same_base_path(self, mock_system):
...
class TestFFmpegUtilsIntegration:
    ...
    @patch('platform.system')
    def test_platform_specific_extensions(self, mock_system):
