...
import pytest
from PySide6.QtCore import QThreadPool, QObject, Qt, QMetaObject, Q_ARG, QRunnable

from app.data_classes import Cloudcast, MixcloudUser
from app.services.download_manager import DownloadManager, CallbackBridge
...
class StubDownloadWorker(QRunnable):
    ...
    def __init__(self, cloudcast, download_dir, callback_bridge, settings_manager):
...
class StubConversionWorker(QRunnable):
    ...
    def __init__(self, cloudcast_url, input_file, target_format, download_dir, 
                 callback_bridge, settings_manager, license_manager):
...
def sample_cloudcasts():
    ...
    return [
        Cloudcast(
            name="Test Mix 1",
            url="https://mixcloud.com/test/mix1",
            user=user1
        ),
        Cloudcast(
            name="Test Mix 2",
            url="https://mixcloud.com/test/mix2",
            user=user2
        )
    ]
...
class TestDownloadManagerStructure:
    ...
    def test_download_manager_initialization(self, download_manager, stub_settings_manager, stub_license_manager):
...
class TestCallbackBridge:
    ...
    def test_emit_completed(self, mock_invoke, download_manager):
        ...
        bridge.emit_completed("https://mixcloud.com/test/mix", "/path/to/file.webm", "download")
...
class TestDownloadWorkflow:
    ...
    def test_start_downloads_multiple_cloudcasts(self, download_manager, sample_cloudcasts, temp_dir):
    ...
    def test_start_downloads_duplicate_prevention(self, download_manager, sample_cloudcasts, temp_dir):
...
class TestSignalEmissionSlots:
    ...
    def test_emit_progress_signal_slot(self, download_manager):
        ...
        download_manager._emit_progress_signal("https://mixcloud.com/test/mix", "Progress 50%", "download")
        
        signal_mock.assert_called_once_with("https://mixcloud.com/test/mix", "Progress 50%")
        
    def test_emit_completed_signal_download_no_conversion(self, download_manager, stub_license_manager, temp_dir):
        ...
        result_signal.assert_called_once_with("https://mixcloud.com/test/mix", str(temp_dir / "test.mp3"), False)
    ...
    def test_emit_completed_signal_download_with_conversion(self, download_manager, stub_license_manager, temp_dir):
        ...
        # Call completion slot for .webm file (different from target mp3)
        download_manager._emit_completed_signal(
            "https://mixcloud.com/test/mix", 
            str(temp_dir / "test.webm"),
            "download"
        )
        
        # Should emit result signal immediately with will_convert=True
        result_signal.assert_called_once_with("https://mixcloud.com/test/mix", str(temp_dir / "test.webm"), True)
    ...
    def test_emit_completed_signal_conversion(self, download_manager):
        ...
        download_manager._emit_completed_signal(
            "https://mixcloud.com/test/mix", 
            "/path/to/converted.mp3",
            "conversion"
        )
        
        # Should emit result signal and remove from tracking
        result_signal.assert_called_once_with("https://mixcloud.com/test/mix", "/path/to/converted.mp3", False)
    ...
    def test_emit_error_signal_download(self, download_manager):
        ...
        error_signal.assert_called_once_with("https://mixcloud.com/test/mix", "Download failed: Network error")
        finished_signal.assert_called_once()
        assert "https://mixcloud.com/test/mix" not in download_manager.active_downloads
    ...
    def test_emit_error_signal_conversion(self, download_manager):
        ...
        # Should emit error signal and remove from tracking
        error_signal.assert_called_once_with("https://mixcloud.com/test/mix", "Conversion failed: FFmpeg error")
        finished_signal.assert_called_once()
        assert "https://mixcloud.com/test/mix" not in download_manager.active_conversions
...
class TestWorkflowManagement:
    ...
    def test_workflow_started_emitted_from_idle(self, download_manager, sample_cloudcasts, temp_dir):
...
class TestDownloadManagerIntegration:
    ...
    def test_complete_pro_user_workflow(self, download_manager, stub_license_manager, sample_cloudcasts, temp_dir):
        ...
        with patch('app.services.download_worker.DownloadWorker', StubDownloadWorker):
            with patch('app.services.conversion_worker.ConversionWorker', StubConversionWorker):
                ...
                download_manager._emit_completed_signal(
                    sample_cloudcasts[0].url,
                    str(temp_dir / "test.webm"),
                    "download"
                )
                
                # Should emit download result with will_convert=True, then start conversion
                task_result.assert_called_once_with(sample_cloudcasts[0].url, str(temp_dir / "test.webm"), True)
                ...
                task_result.assert_called_once_with(sample_cloudcasts[0].url, str(temp_dir / "test.mp3"), False)
                workflow_finished.assert_called_once()
                assert len(download_manager.active_conversions) == 0
        
    def test_complete_free_user_workflow(self, download_manager, stub_license_manager, sample_cloudcasts, temp_dir):
        ...
        with patch('app.services.download_worker.DownloadWorker', StubDownloadWorker):
            ...
            task_result.assert_called_once_with(sample_cloudcasts[0].url, str(temp_dir / "test.webm"), False)
            workflow_finished.assert_called_once()
            assert len(download_manager.active_downloads) == 0
            assert len(download_manager.active_conversions) == 0
...
class TestConversionTriggerLogic:
    ...
    def test_conversion_needed_different_formats(self, download_manager, stub_license_manager, temp_dir):
        """Test conversion triggered when formats differ."""
        stub_license_manager.configure_as_pro_user()
        
        # Settings return mp3 as target format
        with patch('app.services.conversion_worker.ConversionWorker', StubConversionWorker):
            # Call with .webm file (different from mp3 target)
            download_manager.active_downloads["test_url"] = Mock()
            download_manager._emit_completed_signal("test_url", str(temp_dir / "test.webm"), "download")
            
            # Should start conversion
            assert "test_url" in download_manager.active_conversions
            
    def test_conversion_skipped_same_formats(self, download_manager, stub_license_manager, temp_dir):
    ...
    def test_conversion_skipped_free_user(self, download_manager, stub_license_manager, temp_dir):
        ...
        # Call with .webm file (different from mp3 target) as free user
        download_manager.active_downloads["test_url"] = Mock()
        download_manager._emit_completed_signal("test_url", str(temp_dir / "test.webm"), "download")
        
        # Should NOT start conversion, emit result immediately
        assert "test_url" not in download_manager.active_conversions
        result_signal.assert_called_once_with("test_url", str(temp_dir / "test.webm"), False)
