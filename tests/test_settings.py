...
class TestSettingsManager:
    ...
    def test_init_creates_qsettings_instance(self):
        ...
        with patch("app.services.settings_manager.QSettings") as mock_qsettings:
            ...
            mock_qsettings.assert_called_once_with(
                "mixcloud-bulk-downloader", "Mixcloud Bulk Downloader"
            )
    ...
    def test_multiple_settings_instances_independent(self):
        ...
        with patch(
            "app.services.settings_manager.QSettings",
            side_effect=[mock_settings_1, mock_settings_2],
        ):
    ...
    def test_qsettings_integration_details(self):
        ...
        with patch(
            "app.services.settings_manager.QSettings", return_value=mock_settings
        ) as mock_qsettings:
            settings = SettingsManager()
...
class TestSettingsManagerCredentials:
    ...
    def test_email_property_getter(self, fresh_settings):
        ...
        with patch.object(
            settings_instance, "_retrieve_credential", return_value="test@example.com"
        ) as mock_retrieve:
    ...
    def test_license_key_property_getter(self, fresh_settings):
        ...
        with patch.object(
            settings_instance, "_retrieve_credential", return_value="abc123"
        ) as mock_retrieve:
    ...
    def test_last_successful_verification_getter(self, fresh_settings):
        ...
        mock_qsettings.value.assert_called_once_with(
            "last_successful_verification", 0.0, type=float
        )
...
class TestCredentialStorage:
    ...
    def test_store_credential_keyring_import_error_fallback(self, fresh_settings):
        ...
        with patch(
            "app.services.settings_manager.keyring.set_password",
            side_effect=ImportError("No keyring module"),
        ):
            with patch("builtins.print") as mock_print:
                settings_instance._store_credential("test_key", "test_value")
    ...
    def test_retrieve_credential_qsettings_only(self, fresh_settings):
        ...
        with patch(
            "app.services.settings_manager.keyring.get_password",
            side_effect=ImportError("No keyring module"),
        ):
            with patch("builtins.print") as mock_print:
                result = settings_instance._retrieve_credential("test_key", "default")
...
class TestLocalSecretsStorage:
    ...
    def development_settings(self, tmp_path):
        ...
        with (
            patch("app.services.settings_manager.QSettings") as mock_qsettings,
...
class TestAudioConversionSetting:
    """Test cases for audio conversion setting constant."""

    def test_enable_audio_conversion_setting_constant(self):
        """Test that audio conversion setting constant is defined correctly."""
        assert SETTING_ENABLE_AUDIO_CONVERSION == "enable_audio_conversion"
        assert isinstance(SETTING_ENABLE_AUDIO_CONVERSION, str)

    def test_default_enable_audio_conversion_value(self):
        """Test that default audio conversion value is False."""
        assert DEFAULT_ENABLE_AUDIO_CONVERSION is False
        assert isinstance(DEFAULT_ENABLE_AUDIO_CONVERSION, bool)
