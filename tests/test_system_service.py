...
class TestCpuDetection:
    ...
    def test_get_cpu_count_handles_not_implemented_error(self):
        ...
        with patch(
            "app.services.system_service.multiprocessing.cpu_count", side_effect=NotImplementedError
        ):
            result = get_cpu_count()
            assert result == 4
    ...
    def test_get_cpu_count_handles_generic_exception(self):
        ...
        with patch(
            "app.services.system_service.multiprocessing.cpu_count", side_effect=RuntimeError
        ):
            result = get_cpu_count()
            assert result == 4
