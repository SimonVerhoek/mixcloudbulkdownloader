...
from app.services.conversion_worker import ConversionWorker, ConversionCancelled
...
@pytest.fixture
def conversion_worker(test_input_file, temp_dir, stub_callback_bridge, stub_settings_manager, stub_license_manager):
...
class TestConversionWorkerStructure:
    ...
    def test_conversion_worker_file_paths(self, conversion_worker, temp_dir):
        ...
        assert conversion_worker.converting_file_path == Path(temp_dir) / ".converting" / "test_audio.mp3"
...
class TestConversionWorkerPrerequisites:
    ...
    def test_pro_license_required(self, conversion_worker, stub_license_manager, stub_callback_bridge):
    ...
    def test_input_file_validation(self, temp_dir, stub_callback_bridge, stub_settings_manager, stub_license_manager):
    ...
    def test_empty_input_file_validation(self, temp_dir, stub_callback_bridge, stub_settings_manager, stub_license_manager):
...
class TestFFmpegCommandBuilding:
    ...
    def test_build_mp3_command(self, conversion_worker):
        ...
        expected_elements = [
            "/usr/bin/ffmpeg",
            "-y",  # Overwrite
            "-i", conversion_worker.input_file,  # Input
            "-vn",  # No video
            "-c:a", "libmp3lame",  # MP3 codec
            "-b:a", "192k",  # Bitrate for lossy
    ...
    def test_build_flac_command(self, test_input_file, temp_dir, stub_callback_bridge, stub_settings_manager, stub_license_manager):
    ...
    def test_unsupported_format_error(self, test_input_file, temp_dir, stub_callback_bridge, stub_settings_manager, stub_license_manager):
...
class TestConversionWorkerIntegration:
    ...
    def test_successful_conversion_workflow(self, mock_ffmpeg_path, mock_popen, conversion_worker, stub_callback_bridge, temp_dir):
    ...
    @patch('app.services.conversion_worker.subprocess.Popen')
    @patch('app.services.conversion_worker.get_ffmpeg_path')
    def test_conversion_failure_handling(self, mock_ffmpeg_path, mock_popen, conversion_worker, stub_callback_bridge):
...
class TestConversionWorkerEdgeCases:
    ...
    def test_conversion_with_special_characters_in_filename(self, temp_dir, stub_callback_bridge, stub_settings_manager, stub_license_manager):
    ...
    def test_conversion_with_very_long_filename(self, temp_dir, stub_callback_bridge, stub_settings_manager, stub_license_manager):
